#!/usr/bin/env python3
"""
Module d'intégration hybride combinant le système actuel et Grover
...
from grover_search import grover_retrieve_top_k
...
class SearchStrategy(Enum):
    ...
    GROVER_ONLY = "grover_only"              # Grover pur
    HYBRID_ADAPTIVE = "hybrid_adaptive"      # Hybride adaptatif
    GROVER_HYBRID = "grover_hybrid"          # Grover + quantum overlap

class HybridQuantumSearch:
    """
    Système de recherche hybride combinant différentes stratégies quantiques
    """
    ...
    def adaptive_strategy_selection(self, query_length: int, database_size: int) -> SearchStrategy:
        ...
        if database_size < 1000:
            ...
            # Grande base : Grover plus avantageux
    ...
    @time_operation("hybrid_search_execution")
    def search(self, query_text: str, db_folder: str, k: int = 5, 
        ...
        try:
            ...
            elif strategy == SearchStrategy.GROVER_ONLY:
                results = self._execute_grover_only(query_text, db_folder, k, n_qubits, cassandra_manager)
    ...
    def _execute_grover_only(self, query_text: str, db_folder: str, k: int, 
                           n_qubits: int, cassandra_manager) -> List[Tuple[float, str, str]]:
        """Exécuter Grover uniquement"""
        logger.info("🚀 Exécution Grover pur")
        return grover_retrieve_top_k(query_text, db_folder, k, n_qubits, cassandra_manager)
    
    def _execute_grover_hybrid(self, query_text: str, db_folder: str, k: int, 
                             n_qubits: int, cassandra_manager) -> List[Tuple[float, str, str]]:
        """Exécuter Grover + quantum overlap"""
        logger.info("⚡ Exécution Grover hybride")
        
        # Phase 1: Grover pour la sélection rapide
        with time_operation_context("grover_candidate_selection"):
            grover_results = grover_retrieve_top_k(query_text, db_folder, k*3, n_qubits, cassandra_manager)
        ...
        with time_operation_context("quantum_overlap_refinement"):
            # Ici on pourrait ajouter une étape de raffinement quantique
    ...
    def _execute_hybrid_adaptive(self, query_text: str, db_folder: str, k: int, 
        ...
        logger.info("🎯 Exécution hybride adaptative")
        ...
        with time_operation_context("parallel_search"):
            ...
            # Recherche Grover
            grover_results = self._execute_grover_only(query_text, db_folder, k, n_qubits, cassandra_manager)
    ...
    def _merge_results(self, classical_results: List[Tuple[float, str, str]], 
        ...
        Args:
            ...
            grover_results: Résultats de Grover
        ...
        # Ajouter les résultats Grover avec poids 0.8 (pour équilibrer)
...
def hybrid_retrieve_top_k(query_text: str, db_folder: str, k: int = 5, 
                         n_qubits: int = 8, cassandra_manager=None,
    ...
    hybrid_search = HybridQuantumSearch(strategy=search_strategy)
