#!/usr/bin/env python3
"""
Version HYBRIDE de Grover : Recherche vectorielle + Grover
1. Recherche vectorielle classique pour sélectionner 300 candidats
2. Grover quantique sur ces 300 candidats
"""
...
class HybridGroverSearch:
    """
    Version HYBRIDE de Grover : Recherche vectorielle + Grover quantique
    """
    
    def __init__(self, n_qubits: int = 8, threshold: float = 0.4, 
                 classical_candidates: int = 300):
        """
        Initialiser le système de recherche Grover hybride
        
        Args:
            n_qubits: Nombre de qubits pour l'encodage
            threshold: Seuil de similarité pour marquer un document comme pertinent
            classical_candidates: Nombre de candidats à sélectionner classiquement
        """
        self.n_qubits = n_qubits
        self.threshold = threshold
        self.classical_candidates = classical_candidates
        self.backend = Aer.get_backend('statevector_simulator')
        self.shots = 1024
        
    @time_operation("classical_vector_search")
    def classical_vector_search(self, query_embedding: np.ndarray, 
                              document_embeddings: List[np.ndarray],
                              chunk_mapping: List[str]) -> List[Tuple[int, float, str]]:
        """
        Recherche vectorielle classique pour sélectionner les meilleurs candidats
        
        Args:
            query_embedding: Embedding de la requête
            document_embeddings: Liste des embeddings des documents
            chunk_mapping: Mapping des indices vers les chunk_ids
            
        Returns:
            Liste des candidats (index, similarité, chunk_id)
        """
        logger.info(f"🔍 Recherche vectorielle classique sur {len(document_embeddings)} documents")
        ...
        for i, doc_embedding in enumerate(document_embeddings):
            ...
            similarities.append((i, similarity, chunk_mapping[i]))
        
        # Trier par similarité décroissante
        similarities.sort(key=lambda x: x[1], reverse=True)
        
        # Sélectionner les meilleurs candidats
        candidates = similarities[:self.classical_candidates]
        
        logger.info(f"📊 Top {len(candidates)} candidats sélectionnés")
        logger.info(f"   Similarité max: {candidates[0][1]:.4f}")
        logger.info(f"   Similarité min: {candidates[-1][1]:.4f}")
    ...
    def calculate_original_similarities(self, query_embedding: np.ndarray, 
        ...
        """
        Calculer les similarités originales pour les candidats sélectionnés
        
        Args:
            query_embedding: Embedding de la requête
            document_embeddings: Liste des embeddings des candidats
        ...
        logger.info(f"📊 Similarités candidats: min={similarities.min():.3f}, max={similarities.max():.3f}")
        return similarities
    ...
    def adaptive_grover_search(self, similarities: np.ndarray, 
        ...
        """
        Recherche Grover adaptative sur les candidats sélectionnés
        ...
        logger.info(f"🔍 Grover adaptatif: {n_docs} candidats, {num_solutions} solutions, seuil {threshold}")
        ...
        # Stratégie adaptative optimisée: utiliser le nombre optimal pour 300 docs
        max_iterations = min(10, int(np.sqrt(n_docs)))
        best_results = []
        best_confidence = 0
    ...
    def search_documents(self, query_embedding: np.ndarray, 
        ...
        """
        Recherche hybride : classique + Grover
        ...
        Returns:
            Liste des documents trouvés (index, similarité, chunk_id)
        """
        logger.info(f"🚀 Recherche hybride sur {len(document_embeddings)} documents")
        
        # 1. Recherche vectorielle classique pour sélectionner les candidats
        candidates = self.classical_vector_search(query_embedding, document_embeddings, chunk_mapping)
        
        if not candidates:
            return []
        
        # 2. Extraire les embeddings des candidats
        candidate_embeddings = [document_embeddings[idx] for idx, _, _ in candidates]
        candidate_mapping = [chunk_id for _, _, chunk_id in candidates]
        
        # 3. Calculer les similarités pour les candidats
        similarities = self.calculate_original_similarities(query_embedding, candidate_embeddings)
        
        # 4. Exécuter la recherche Grover sur les candidats
        grover_results = self.adaptive_grover_search(similarities, self.threshold)
        
        # 5. Mapper les résultats vers les chunk_ids originaux
        final_results = []
        for doc_index, similarity in grover_results:
            if doc_index < len(candidate_mapping):
                chunk_id = candidate_mapping[doc_index]
                final_results.append((doc_index, similarity, chunk_id))
        
        logger.info(f"🔍 Recherche hybride trouvé {len(final_results)} documents pertinents")
    ...
    def hybrid_grover_search(self, query_text: str, cassandra_manager, 
        ...
        """
        Recherche hybride CORRIGÉE combinant recherche classique et Grover
        ...
        """
        logger.info(f"🚀 Recherche hybride classique+Grover pour: '{query_text[:50]}...'")
        ...
        # 3. Exécuter la recherche hybride
        ...
        logger.info(f"🔍 Recherche hybride trouvé {len(hybrid_results)} documents pertinents")
        
        # 4. Convertir les résultats en circuits QASM
        qasm_results = []
        for doc_index, similarity, chunk_id in hybrid_results:
        ...
        logger.info(f"✅ {len(qasm_results)} circuits QASM trouvés pour recherche hybride")
        return qasm_results[:k]

# Fonction de compatibilité avec l'API existante
def hybrid_grover_retrieve_top_k(query_text: str, db_folder: str, k: int = 5, 
                               n_qubits: int = 8, cassandra_manager=None) -> List[Tuple[float, str, str]]:
    """
    Interface de compatibilité pour la recherche hybride classique+Grover
    ...
    if cassandra_manager is None:
        logger.error("❌ Cassandra manager requis pour Grover hybride")
        return []
    
    grover_search = HybridGroverSearch(n_qubits=n_qubits, classical_candidates=300)
