...
async def configure_grover(config: dict):
    ...
    try:
        if "enabled" in config:
            api_instance.use_grover = config["enabled"]
        if "strategy" in config:
            api_instance.grover_strategy = config["strategy"]
        if "threshold" in config:
            api_instance.grover_threshold = config["threshold"]
        
        return {"status": "success", "config": {
            "use_grover": api_instance.use_grover,
            "strategy": api_instance.grover_strategy,
...
async def get_grover_stats():
    ...
    try:
        stats = {
            "grover_enabled": api_instance.use_grover,
            "current_strategy": api_instance.grover_strategy,
            "performance_history": api_instance.performance_history,
            "config": {
                "enabled": api_instance.use_grover,
                "strategy": api_instance.grover_strategy,
                "threshold": api_instance.grover_threshold
            }
        }
